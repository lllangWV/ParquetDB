...
def main():
    ...
    with open(
        os.path.join(
            banchmark_dir, "alexandria", "create_load_benchmark_alexandria_3D.json"
        ),
        "r",
    ) as f:
    ...
    with open(
        os.path.join(banchmark_dir, "alexandria", "task_benchmark_alexandria_3D.json"),
        "r",
    ) as f:
        task_data = json.load(f)

    # plot_load_create_times(data['create_times'],data['json_load_times'],data['n_rows_per_file'])
    plot_load_create_times(
        create_load_data["create_times"],
        create_load_data["json_load_times"],
        create_load_data["n_rows_per_file"],
    ...
    operations_plot(
        ...
        task_data["task_times"],
        savefig=os.path.join(
            banchmark_dir, "alexandria_database_operations_benchmark.png"
        ),
    )
    operations_plot(
        task_data["task_names"],
        task_data["task_times"],
        savefig=os.path.join(
            banchmark_dir, "alexandria_database_operations_benchmark.pdf"
        ),
    )
...
def plot_load_create_times(
    create_times, json_load_times, n_rows_per_file, savefig=None
):
    ...
    ax1.bar(
        range(len(json_load_times)),
        json_load_times,
        color="#59b9de",
        label="JSON Load Times",
        alpha=0.6,
    )

    # Plot create_times
    ax1.bar(
        range(len(create_times)),
        create_times,
        color="#e52207",
        label="Create Times",
        alpha=0.6,
    ...
    ax1.axhline(
        ...
        color="#e52207",
        linestyle="--",
        label=f"Avg Create Time: {avg_create_time:.2f} s",
        linewidth=2,
    )
    ax1.axhline(
        avg_json_load_time,
        color="#59b9de",
        linestyle="--",
        label=f"Avg JSON Load Time: {avg_json_load_time:.2f} s",
        linewidth=2,
    )
    ...
    ax1.set_title("Create Times and JSON Load Times")
