...
def generate_needle_in_haystack_data(n_rows=100, n_columns=100, value=-1):
    ...
    for j in range(n_rows):
        if j == random_index:
            ...
            data.append(
                {f"col_{i}": random.randint(0, 100000) for i in range(n_columns)}
            )
    return data


def benchmark_needle_in_haystack(num_rows, db_path):
    ...
    start_time = time.time()
    data = db.read(
        columns=["col_0"], filters=[pc.field("col_0") == -1]
    ).combine_chunks()
...
if __name__ == "__main__":
    ...
    for num_rows in row_counts:
        ...
        insert_time, read_time = benchmark_needle_in_haystack(num_rows, db_path=db_path)
    ...
    df.to_csv(
        os.path.join(save_dir, "parquetdb_needle_in_haystack_benchmark.csv"),
        index=False,
    )
